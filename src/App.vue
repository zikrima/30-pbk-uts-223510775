...
export default {
  ...
  computed: {
    tugasBelumSelesai() {
      if (this.tampilkanBelumSelesaiSaja) {
        return this.daftarTugas.filter(tugas => !tugas.completed); // Menyaring kegiatan yang belum selesai
      }
  ...
  methods: {
    ...
    hapusTugas(tugas) {
      this.daftarTugas = this.daftarTugas.filter(item => item.id !== tugas.id); // Menghapus tugas dari daftar
    },
    filterBelumSelesai() {
      this.tampilkanBelumSelesaiSaja = !this.tampilkanBelumSelesaiSaja; // Mengubah status tampilkanBelumSelesaiSaja
    }
  }
}
</script>
